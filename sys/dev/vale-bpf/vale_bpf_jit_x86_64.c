...
static int translate(struct vale_bpf_vm *vm, struct jit_state *state) {
  emit_push(state, RBP);
  emit_push(state, RBX);
  emit_push(state, R13);
  emit_push(state, R14);
  ...
  for (i = 0; i < vm->num_insts; i++) {
    ...
    switch (inst.opcode) {
      ...
      default:
        D("Unknown instruction at PC %d: opcode %02x", i, inst.opcode);
        return -1;
    }
  }
  ...
  // const char *div_by_zero_fmt = "vale-bpf error: division by zero at PC
  // %u\n";
...
static void resolve_jumps(struct vale_bpf_vm *vm, struct jit_state *state) {
  int i;
...
vale_bpf_jit_fn vale_bpf_compile(struct vale_bpf_vm *vm) {
  ...
  if (!vm->insts) {
    D("code has not been loaded into this VM");
    return NULL;
  }
  ...
  state.pc_locs =
      vale_bpf_os_malloc((MAX_INSTS + 1) * sizeof(state.pc_locs[0]));
