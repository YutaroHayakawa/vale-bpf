...
static inline void emit_bytes(struct jit_state *state, void *data,
                              uint32_t len) {
  KASSERT(state->offset <= state->size - len, "");
...
static inline void emit_jump_offset(struct jit_state *state,
                                    int32_t target_pc) {
...
static inline void emit_modrm_and_displacement(struct jit_state *state, int r,
                                               int m, int32_t d) {
  if (d == 0 && (m & 7) != RBP) {
...
static inline void emit_rex(struct jit_state *state, int w, int r, int x,
                            int b) {
  KASSERT(!(w & ~1), "");
  KASSERT(!(r & ~1), "");
  KASSERT(!(x & ~1), "");
 ...
 */
static inline void emit_basic_rex(struct jit_state *state, int w, int src,
                                  int dst) {
  if (w || (src & 8) || (dst & 8)) {
...
static inline void emit_alu32(struct jit_state *state, int op, int src,
                              int dst) {
...
static inline void emit_alu32_imm32(struct jit_state *state, int op, int src,
                                    int dst, int32_t imm) {
...
static inline void emit_alu32_imm8(struct jit_state *state, int op, int src,
                                   int dst, int8_t imm) {
  emit_alu32(state, op, src, dst);
  emit1(state, imm);
}
...
/* We use the MR encoding when there is a choice */
/* 'src' is often used as an opcode extension */
static inline void emit_alu64(struct jit_state *state, int op, int src,
                              int dst) {
...
static inline void emit_alu64_imm32(struct jit_state *state, int op, int src,
                                    int dst, int32_t imm) {
...
static inline void emit_alu64_imm8(struct jit_state *state, int op, int src,
                                   int dst, int8_t imm) {
  emit_alu64(state, op, src, dst);
  emit1(state, imm);
}
...
static inline void emit_cmp_imm32(struct jit_state *state, int dst,
                                  int32_t imm) {
...
static inline void emit_jcc(struct jit_state *state, int code,
                            int32_t target_pc) {
  emit1(state, 0x0f);
  emit1(state, code);
...
static inline void emit_load(struct jit_state *state, enum operand_size size,
                             int src, int dst, int32_t offset) {
...
static inline void emit_load_imm(struct jit_state *state, int dst,
                                 int64_t imm) {
...
static inline void emit_store(struct jit_state *state, enum operand_size size,
                              int src, int dst, int32_t offset) {
  if (size == S16) {
...
static inline void emit_store_imm32(struct jit_state *state,
                                    enum operand_size size, int dst,
