...
static int
translate(struct vale_bpf_vm *vm, struct jit_state *state)
{
    emit_push(state, RBP);
    emit_push(state, RBX);
    emit_push(state, R13);
    ...
    // const char *div_by_zero_fmt = "vale-bpf error: division by zero at PC %u\n";
...
static void
resolve_jumps(struct vale_bpf_vm *vm, struct jit_state *state)
{
    int i;
...
vale_bpf_jit_fn
vale_bpf_compile(struct vale_bpf_vm *vm)
{
    void *jitted = NULL;
    size_t jitted_size;
